...
describe('CLI LLM Flags (Phase 4 WS-F2 Stage C)', () => {
    ...
    describe('--llm-provider flag', () => {
        ...
        it('should reject unsupported provider with descriptive error', () => {
            ...
            expect(() => validateArgs(args)).toThrow('Invalid provider: invalid-provider. Supported: anthropic, openai, azure, local');
        });
        it('should throw error if --llm-provider has no value', () => {
            expect(() => parseArgs(['node', 'ceps', '--llm-provider'])).toThrow('--llm-provider requires a value');
        });
    });
    describe('--llm-model flag', () => {
        ...
        it('should throw error if --llm-model has no value', () => {
            expect(() => parseArgs(['node', 'ceps', '--llm-model'])).toThrow('--llm-model requires a value');
        });
    });
    describe('--llm-budget flag', () => {
        ...
        it('should throw error if --llm-budget has no value', () => {
            expect(() => parseArgs(['node', 'ceps', '--llm-budget'])).toThrow('--llm-budget requires a value');
        });
    });
    describe('--no-llm-cache flag', () => {
        ...
        it('should warn when --llm is off', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith('Warning: --llm is off; ignoring --no-llm-cache');
        });
    ...
    describe('Flag interactions', () => {
        it('should warn when --llm off with --llm-provider', () => {
            ...
            validateArgs(args);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Warning: --llm is off; ignoring --llm-provider'));
        ...
        it('should warn when --llm off with --llm-model', () => {
            ...
            validateArgs(args);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Warning: --llm is off; ignoring --llm-model'));
            // Verify flag was cleared
        ...
        it('should warn when --llm off with --llm-budget', () => {
            ...
            validateArgs(args);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Warning: --llm is off; ignoring --llm-budget'));
            // Verify flag was cleared
        ...
        it('should warn when --llm off with multiple llm flags', () => {
            ...
            validateArgs(args);
            expect(consoleWarnSpy).toHaveBeenCalledWith('Warning: --llm is off; ignoring --llm-provider, --llm-model, --llm-budget, --no-llm-cache');
    ...
    describe('--llm flag parsing', () => {
        ...
        it('should throw error if --llm has invalid value', () => {
            expect(() => parseArgs(['node', 'ceps', '--llm', 'invalid'])).toThrow('--llm must be either "on" or "off"');
        });
...
//# sourceMappingURL=cli-llm-flags.test.js.map