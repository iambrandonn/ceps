...
describe('IdentifierValidator', () => {
    ...
    describe('Relation Validation', () => {
        it('should detect missing call relation', () => {
            ...
            const result = validator.validateRelations('class-UserService', ['unknownFunction'], ['fs-user-service']);
            expect(result.valid).toBe(false);
    ...
    describe('Mixed Valid/Invalid', () => {
        it('should reject when any identifier is invalid', () => {
            const result = validator.validate(['UserService', 'UnknownService', 'saveUser'], ['fs-user-service']);
            expect(result.valid).toBe(false);
            expect(result.diagnostics.length).toBeGreaterThan(0);
            expect(result.diagnostics.some(d => d.reason.includes('UnknownService'))).toBe(true);
        });
        it('should collect all validation errors', () => {
            const result = validator.validate(['Unknown1', 'Unknown2', 'Unknown3'], ['fs-user-service']);
            expect(result.valid).toBe(false);
            expect(result.diagnostics).toHaveLength(3);
        });
    });
});
//# sourceMappingURL=validator-identifiers.test.js.map