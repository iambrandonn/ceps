...
export class MockValidator {
    ...
    setNextResult(result) {
        // Validate status
        const validStatuses = ['accept', 'retry', 'fallback'];
        ...
        if (result.retryMetadata) {
            ...
            if (!validAttempts.includes(result.retryMetadata.attempt)) {
                throw new Error(`Invalid retry attempt: ${result.retryMetadata.attempt}. Must be 0, 1, or 2`);
            }
            const validPromptKeys = ['O', 'R1', 'R2'];
            if (!validPromptKeys.includes(result.retryMetadata.promptKey)) {
                throw new Error(`Invalid promptKey: ${result.retryMetadata.promptKey}. Must be O, R1, or R2`);
            }
        }
     ...
     */
    validate(_draft, _factSetIds, _metadata) {
        return this.nextResult;
    }
}
//# sourceMappingURL=mock-validator.js.map