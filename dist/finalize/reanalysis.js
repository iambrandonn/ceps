...
import { verifySnapshot } from '../snapshot/index.js';
...
export async function reanalyzeEntities(kb, impactReport, options) {
    const start = performance.now();
    const updatedChunks = new Map();
    const failedEntities = [];
    const warnings = new Set(impactReport.diagnostics.warnings);
    ...
    for (const entityId of processedEntities) {
        try {
            ...
            if (!entity) {
                recordFailure(failedEntities, entityId, 'kb-inconsistency', 'Entity not found in KnowledgeBase');
                continue;
            }
            ...
            if (!originalChunk && !options.reasoningEnabled) {
                recordFailure(failedEntities, entityId, 'kb-inconsistency', 'No existing chunk and reasoning disabled', originalChunk);
                continue;
            }
            if (options.reasoningEnabled && factSets.length > 0 && lifter) {
                try {
                    ...
                    recordFailure(failedEntities, entityId, 'kb-inconsistency', error instanceof Error ? error.message : String(error), originalChunk);
                    continue;
                }
            }
            if (!originalChunk && !reasonedChunk) {
                recordFailure(failedEntities, entityId, 'kb-inconsistency', 'Unable to build behavior chunk for entity', originalChunk);
                continue;
            }
            ...
            if (!hasAnswers) {
                try {
                    ...
                    if (error instanceof PipelineFailure) {
                        ...
                        error.diagnostics?.forEach((diag) => warnings.add(`Grounding failure for ${entityId}: ${diag.reason}`));
                        continue;
                    }
                    recordFailure(failedEntities, entityId, 'llm-failure', error instanceof Error ? error.message : String(error), originalChunk);
                    continue;
                }
            }
                ...
                const validation = options.validator.validate(finalChunk.textDraft, metadata.factSetIds, metadata);
                if (validation.status === 'fallback') {
                    recordFailure(failedEntities, entityId, 'grounding-reject', validation.diagnostics[0]?.reason ?? 'Grounding validator rejected human answer', originalChunk);
                    continue;
                }
                if (validation.status === 'retry') {
                    warnings.add(`Validator requested retry for ${entityId}, but human answer prevented automatic retry.`);
                }
            }
            updatedChunks.set(entityId, finalChunk);
        }
        catch (error) {
            recordFailure(failedEntities, entityId, 'kb-inconsistency', error instanceof Error ? error.message : String(error));
        }
    }
...
async function runLLMPipeline(params) {
    ...
    if (options.budgetTracker) {
        ...
        if (!budgetCheck.allowed) {
            throw new PipelineFailure('llm-failure', `LLM budget exhausted while processing ${entity.id} (remaining tokens: ${budgetCheck.remaining})`);
        }
    }
    let attempt = 0;
    let promptKey = 'O';
    ...
    while (attempt < maxAttempts) {
        try {
            ...
            if (validation.status === 'retry' && attempt < maxAttempts - 1) {
                ...
                guidance.push(...validation.diagnostics.map((diag) => diag.reason).filter((reason) => !!reason));
                continue;
            }
            throw new PipelineFailure('grounding-reject', validation.diagnostics[0]?.reason ??
                `Grounding validator rejected chunk for ${entity.id}`, validation.diagnostics);
        }
        catch (error) {
            if (error instanceof PipelineFailure) {
                throw error;
            }
            throw new PipelineFailure('llm-failure', error instanceof Error ? error.message : String(error));
        }
    }
    throw new PipelineFailure('grounding-reject', `Grounding validator failed after ${maxAttempts} attempts for ${entity.id}`);
}
function maybeValidateChunk(chunk, entity, validator) {
    if (!validator)
        return;
    const metadata = {
        chunkId: chunk.id,
    ...
    throw new PipelineFailure('grounding-reject', `Grounding validator requested retry for ${entity.id} but LLM pipeline is disabled`, validation.diagnostics);
}
function buildUpdatedChunk(params) {
...
function defaultTemplate(entity) {
    const label = entity.kind === 'function' ? 'Function' : 'Entity';
    return `${label} ${entity.name} (finalization template)`;
}
function determineFactSetIds(originalChunk, reasonedChunk, factSets) {
...
function recordFailure(failures, entityId, reason, details, originalChunk) {
    failures.push({
        entityId,
        reason,
        details,
...
function mapAnswersByEntity(kb, scopedQids) {
...
function computeTokenUsage(budgetTracker, initialUsage) {
    if (!budgetTracker)
        return 0;
...
async function verifyProjectSnapshot(snapshotOptions, warnings) {
    if (!snapshotOptions)
        return;
    ...
    if (verification.match || verification.reconciled) {
        if (!verification.match && verification.reconciled) {
            warnings.add(`Snapshot mismatch reconciled for finalize run (${formatMismatch(verification)}).`);
        }
        return;
    }
    throw new SnapshotMismatchError(verification);
}
function mismatchSummary(verification) {
...
function formatMismatch(verification) {
...
//# sourceMappingURL=reanalysis.js.map