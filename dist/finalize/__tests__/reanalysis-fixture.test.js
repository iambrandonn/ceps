...
import { reanalyzeEntities } from '../reanalysis.js';
...
describe('reanalyzeEntities (fixture)', () => {
    it('matches golden reanalysis output for tiny-react fixture', async () => {
        ...
        const impactReport = JSON.parse(fs.readFileSync(path.join(FIXTURE_ROOT, 'impact.report.json'), 'utf8'));
        const options = {
        ...
        const expected = JSON.parse(fs.readFileSync(path.join(FIXTURE_ROOT, 'reanalysis.success.json'), 'utf8'));
...
//# sourceMappingURL=reanalysis-fixture.test.js.map