...
export class PatternMatcher {
     ...
     */
    matchExpressErrorHandler(factSet) {
        const has4Params = factSet.facts.some(f => f.predicate === 'param-count' && f.object === 4);
     ...
     */
    matchExpressRouteHandler(factSet) {
        const routeRegex = /^app\.(get|post|put|delete|patch)$/;
        const routeCallIndex = factSet.facts.findIndex(f => f.predicate === 'calls-expression' && routeRegex.test(String(f.object)));
        if (routeCallIndex !== -1) {
     ...
     * Priority: 70
     */
    matchExpressMiddleware(factSet) {
        const hasAppUse = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'app.use');
        const has3Params = factSet.facts.some(f => f.predicate === 'param-count' && f.object === 3);
        const paramNames = factSet.facts.find(f => f.predicate === 'param-names');
     ...
     */
    matchReactFunctionalComponent(factSet) {
     ...
     * Priority: 75
     */
    matchReactUseStateHook(factSet) {
        const hasUseState = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useState');
        if (hasUseState) {
            return {
     ...
     * Priority: 75
     */
    matchReactUseEffectHook(factSet) {
        const hasUseEffect = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useEffect');
        if (hasUseEffect) {
            return {
     ...
     * Priority: 75
     */
    matchReactUseContextHook(factSet) {
        const hasUseContext = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useContext');
        if (hasUseContext) {
            return {
     ...
     * Priority: 75
     */
    matchReactUseRefHook(factSet) {
        const hasUseRef = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useRef');
        if (hasUseRef) {
            return {
     ...
     * Priority: 75
     */
    matchReactUseMemoHook(factSet) {
        const hasUseMemo = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useMemo');
        if (hasUseMemo) {
            return {
     ...
     * Priority: 75
     */
    matchReactUseCallbackHook(factSet) {
        const hasUseCallback = factSet.facts.some(f => f.predicate === 'calls-expression' && String(f.object) === 'useCallback');
        if (hasUseCallback) {
            return {
