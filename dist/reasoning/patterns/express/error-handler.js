...
import { getParameterCount, getParameterNames, isAsync, } from '../shared/helpers.js';
export class ExpressErrorHandlerPattern {
    ...
    describe(kb, entity) {
        try {
            ...
            const paramFactSets = factSets.filter(fs => fs.facts.some(f => f.predicate === 'param-count' || f.predicate === 'param-names'));
            // Check for async behavior
            const async = isAsync(kb, entity);
            const asyncFactSets = async ? factSets.filter(fs => fs.facts.some(f => f.predicate === 'is-async' || f.predicate === 'returns-promise')) : [];
