...
import { getFactsByPredicate, getFactSets, getFirstFact, } from '../shared/helpers.js';
...
export class MongooseQueryPattern {
    id = 'mongoose.query';
    ...
    matches(kb, entity) {
        try {
            ...
            return callExprs.some(fact => {
                ...
                return ALL_QUERY_METHODS.some(method => new RegExp(`\\w+\\.${method}\\b`).test(callExpr));
            });
        }
        catch (error) {
    ...
    describe(kb, entity) {
        try {
            ...
            const resolvedCount = operations.filter(op => this.resolveModelEntity(kb, entity, op.modelName) !== null).length;
            let confidence;
     ...
     */
    extractQueryOperations(kb, entity) {
        const operations = [];
        try {
            const callExprs = getFactsByPredicate(kb, entity, 'calls-expression');
     ...
     */
    resolveModelEntity(kb, sourceEntity, modelName) {
        try {
    ...
    getMethodCategory(method) {
        if (QUERY_METHODS.read.includes(method))
            return 'read';
        if (QUERY_METHODS.write.includes(method))
            return 'write';
        if (QUERY_METHODS.aggregate.includes(method))
            return 'aggregate';
        return 'read'; // Default
    }
    /**
...
//# sourceMappingURL=mongoose-query.js.map