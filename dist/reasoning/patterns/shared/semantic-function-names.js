...
export class SemanticFunctionPattern {
    id = 'shared.semantic-function-names';
     ...
     */
    generateDescription(pattern, subject, params) {
        let description = `${pattern.verb} ${subject}`;
        // Add parameter context if available
        if (params.length > 0) {
            // Extract meaningful parameter names
            const meaningfulParams = params.filter(p => !['req', 'res', 'next', 'callback', 'cb', 'options', 'opts'].includes(p.toLowerCase()));
...
//# sourceMappingURL=semantic-function-names.js.map