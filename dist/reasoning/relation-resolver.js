...
export class RelationResolver {
    kb;
    entityLookup = new Map(); // name → [entity IDs]
    importMap = new Map(); // filePath → set of module specifiers
    constructor(kb) {
        this.kb = kb;
    }
    /**
    ...
    resolve(relations) {
        ...
        for (const relation of relations) {
            if (relation.predicate === 'calls') {
                ...
                const entityId = this.resolveCallExpression(relation.objectId || '', relation.subjectId, callerEntity?.path);
                resolved.push({
                    ...relation,
     ...
     */
    resolveCallExpression(expr, callerId, callerPath) {
     ...
     * @returns Entity ID if resolved, null otherwise
     */
    resolveSimpleName(name, callerId, callerPath) {
