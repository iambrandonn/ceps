...
export function normalizeHttpMethod(method: string): HttpMethod | undefined {
  ...
  return HTTP_METHODS.includes(upper as HttpMethod) ? (upper as HttpMethod) : undefined;
}
 ...
 */
export function getParameterNames(kb: KnowledgeBase, entity: Entity): string[] {
 ...
 */
export function getParameterCount(kb: KnowledgeBase, entity: Entity): number {
 ...
 */
export function isAsync(kb: KnowledgeBase, entity: Entity): boolean {
  return (
    hasFact(kb, entity, 'is-async', 'true') ||
 ...
 */
export function getFactSets(kb: KnowledgeBase, entity: Entity): FactSet[] {
  // Query KB for factSets where facts reference this entity
  const allFactSets = kb.getAllFactSets();
  return allFactSets.filter(fs =>
    fs.facts.some(fact => fact.subjectId === entity.id)
  );
}
