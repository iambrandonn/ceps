...
export class MongooseSchemaPattern implements PatternModule {
  id = 'mongoose.schema';
    ...
    try {
      ...
      if (fields.length > 0) {
        const fieldNames = fields.map(f => {
          ...
          if (f.isReference && f.refModel) desc += ` → ${f.refModel}`;
          return desc;
        });
   ...
   */
  private extractFields(initializer: string): SchemaField[] {
    ...
    try {
      ...
      while ((match = complexPattern.exec(schemaBody)) !== null) {
        ...
        const field: SchemaField = { name: fieldName };
   ...
   */
  private determineConfidence(initializer: string, fields: SchemaField[]): 'High' | 'Medium' | 'Low' {
