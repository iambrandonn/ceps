...
describe('LLMGateway.polish()', () => {
  ...
  describe('interface contract', () => {
    it('should accept draftText, entity, and factSets', async () => {
      ...
      vi.spyOn(gateway as any, 'completions').mockResolvedValue(
        'Builds and returns a memoized cache object for performance optimization.'
      );
    ...
    it('should generate prompt with entity context', async () => {
      const completionsSpy = vi.spyOn(gateway as any, 'completions').mockResolvedValue(
    ...
    it('should include factSet evidence in prompt', async () => {
      ...
      await gateway.polish(
        'Function getUser (intent unclear)',
        { id: 'e-3', kind: 'function', name: 'getUser', path: 'src/user.ts' },
        factSets
      );
  ...
  describe('model selection', () => {
    it('should use Haiku model for cost-efficiency', async () => {
      ...
      await gateway.polish(
        'Function foo',
        { id: 'e-4', kind: 'function', name: 'foo', path: 'test.ts' },
        []
      );
  ...
  describe('budget tracking', () => {
    it('should track token usage via completions()', async () => {
      ...
      await gateway.polish(
        'Test draft',
        { id: 'e-5', kind: 'function', name: 'test', path: 'test.ts' },
        []
      );
  ...
  describe('prompt guidelines', () => {
    it('should include behavioral guidelines in prompt', async () => {
      ...
      await gateway.polish(
        'Draft',
        { id: 'e-6', kind: 'function', name: 'test', path: 'test.ts' },
        []
      );
    ...
    it('should discourage "intent unclear" phrasing', async () => {
      ...
      await gateway.polish(
        'Function with unclear intent',
        { id: 'e-7', kind: 'function', name: 'mystery', path: 'test.ts' },
        []
      );
  ...
  describe('error handling', () => {
    it('should propagate API errors from completions()', async () => {
      vi.spyOn(gateway as any, 'completions').mockRejectedValue(
      ...
      await expect(
        gateway.polish(
          'Draft',
          { id: 'e-8', kind: 'function', name: 'test', path: 'test.ts' },
          []
        )
    ...
    it('should propagate budget errors from completions()', async () => {
      ...
      await expect(
        gateway.polish(
          'Draft',
          { id: 'e-9', kind: 'function', name: 'test', path: 'test.ts' },
          []
        )
