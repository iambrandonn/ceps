...
import { parseArgs, validateArgs, type CliArgs, type FileSystem } from '../cli.js';
...
describe('CLI Finalize Flags (Phase 5 Step 6)', () => {
  ...
  const createMockFs = (options: {
    ...
    kbStateExists?: boolean;
    projectRootExists?: boolean;
  } = {}): FileSystem => ({
    existsSync: (p: string) => {
      if (p.includes('missing.md')) return options.answersExists ?? false;
      if (p.includes('kb-state.json')) return options.kbStateExists ?? true;
  ...
  describe('finalize command detection', () => {
    ...
    it('should reject unknown commands', () => {
      expect(() => parseArgs(['node', 'ceps', 'invalid-command'])).toThrow(
        'Unknown command: invalid-command. Supported commands: baseline, finalize'
      );
    });
  });
  ...
  describe('--answers flag', () => {
    ...
    it('should throw error if --answers has no value', () => {
      expect(() => parseArgs(['node', 'ceps', 'finalize', '--answers'])).toThrow(
    ...
    it('should throw error if --answers value starts with --', () => {
      expect(() =>
        parseArgs(['node', 'ceps', 'finalize', '--answers', '--dry-run'])
      ).toThrow('--answers requires a value');
    });
    ...
    it('should error if finalize command used without --answers', () => {
      ...
      expect(() => validateArgs(args)).toThrow(
        'finalize command requires --answers <path>'
      );
    });
  });
  ...
  describe('--reconcile flag', () => {
    ...
    it('should warn if --reconcile and --dry-run both provided', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Warning: --reconcile has no effect in --dry-run mode'
      );
    });
  });
  ...
  describe('--finalize-max-hops flag', () => {
    ...
    it('should throw error if --finalize-max-hops is not a positive integer', () => {
      ...
      expect(() => validateArgs(args)).toThrow(
        '--finalize-max-hops must be a positive integer'
      );
    });
    ...
    it('should throw error if --finalize-max-hops has no value', () => {
      expect(() =>
        parseArgs(['node', 'ceps', 'finalize', '--answers', './answers.md', '--finalize-max-hops'])
      ).toThrow('--finalize-max-hops requires a value');
    });
  });
  ...
  describe('--finalize-max-nodes flag', () => {
    ...
    it('should throw error if --finalize-max-nodes is not a positive integer', () => {
      ...
      expect(() => validateArgs(args)).toThrow(
        '--finalize-max-nodes must be a positive integer'
      );
    });
  });
  ...
  describe('--finalize-scope flag', () => {
    ...
    it('should throw error if --finalize-scope has invalid value', () => {
      ...
      expect(() => validateArgs(args)).toThrow(
        '--finalize-scope must be either "auto" or "full"'
      );
    });
    ...
    it('should warn if --finalize-scope full ignores max-hops/max-nodes', () => {
      ...
      validateArgs(args, mockFs);
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Warning: --finalize-scope full ignores --finalize-max-hops and --finalize-max-nodes'
      );
    });
  });
  ...
  describe('Flag combinations and defaults', () => {
    ...
    it('should not allow baseline-only flags in finalize mode', () => {
      ...
      expect(() => validateArgs(args)).toThrow(
        '--no-snapshot is only valid for baseline command'
      );
    });
  });
  ...
  describe('KB state file validation', () => {
    it('should error if .ceps/kb-state.json does not exist', () => {
      ...
      const args = parseArgs(['node', 'ceps', 'finalize', '--answers', './answers.md']);
      expect(() => validateArgs(args, mockFs)).toThrow(
        'Baseline run required before finalization: .ceps/kb-state.json not found'
      );
    });
