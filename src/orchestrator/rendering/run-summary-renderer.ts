...
import type { RunSummary, GateStatus } from '../types/run-summary.js';
 ...
 */
export function validateRunSummary(summary: RunSummary): boolean {
 ...
 */
export function renderJSON(summary: RunSummary, validateSchema = true): string {
  if (validateSchema) {
 ...
 */
export function renderConsole(summary: RunSummary): string {
  const lines: string[] = [];
  ...
  lines.push('─────────────────────────────────────────────────────────');
  lines.push(formatGateLine('Coverage', summary.gates.coverage.status,
    `${summary.gates.coverage.documented}/${summary.gates.coverage.exported} documented, ${summary.gates.coverage.qids} QIDs`));
  lines.push(formatGateLine('Link', summary.gates.link.status,
    `${summary.gates.link.anchors} anchors, ${summary.gates.link.broken} broken`));
  lines.push(formatGateLine('Grounding', summary.gates.grounding.status,
    `${summary.gates.grounding.chunks} chunks (${summary.gates.grounding.validated} validated, ${summary.gates.grounding.fallback} fallback)`));
  lines.push(formatGateLine('Determinism', summary.gates.determinism.status,
    summary.gates.determinism.status === 'skip' ? 'not enabled' :
    `${summary.gates.determinism.reruns} reruns, ${summary.gates.determinism.diffs} diffs`));
  lines.push(formatGateLine('Confidence', summary.gates.confidence.status,
    `${summary.gates.confidence.openQuestions} open questions`));
  lines.push(formatGateLine('Monorepo', summary.gates.monorepo.status,
    summary.gates.monorepo.packagesLinked === 0 ? 'not a monorepo' :
  ...
  lines.push('─────────────────────────────────────────────────────────');
  lines.push(formatGateLine('Cost', summary.validation.cost.status,
    `${summary.validation.cost.used}/${summary.validation.cost.budget} tokens (${summary.validation.cost.remaining} remaining)`));
  lines.push(formatGateLine('Adversarial', summary.validation.adversarial.status,
    summary.validation.adversarial.total === 0 ? 'no tests' :
    `${summary.validation.adversarial.rejected}/${summary.validation.adversarial.total} rejected`));
  lines.push(formatGateLine('Test Coverage', summary.validation.testCoverage.status,
    `${summary.validation.testCoverage.coverage.toFixed(1)}% (threshold: ${summary.validation.testCoverage.threshold}%)`));
  lines.push(formatGateLine('Readability', summary.validation.readability.status,
    summary.validation.readability.avgScore ?
 ...
 */
function formatGateLine(name: string, status: GateStatus, details: string): string {
 ...
 */
function getExitCodeLabel(exitCode: 0 | 1 | 2 | 3): string {
  switch (exitCode) {
    case 0:
      return 'Success';
    case 1:
 ...
 */
export function writeJSONSummary(summary: RunSummary, filePath: string): void {
...
export function emitRunSummary(
  summary: RunSummary,
  options: { jsonPath?: string; console?: boolean } = {}
): void {
