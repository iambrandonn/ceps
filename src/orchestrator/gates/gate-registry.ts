...
export class GateRegistry {
  private runtimeGates: Map<string, GateEvaluator<any, any>> = new Map();
  private validationGates: Map<string, GateEvaluator<any, any>> = new Map();
   ...
   */
  registerRuntimeGate<TInput, TResult>(
    name: string,
    evaluator: GateEvaluator<TInput, TResult>
  ): void {
   ...
   * @param evaluator - Gate evaluator instance
   */
  registerValidationGate<TInput, TResult>(
    name: string,
    evaluator: GateEvaluator<TInput, TResult>
  ): void {
  ...
  getFailedRuntimeGates(summary: RunSummary): string[] {
    ...
    if (summary.gates.coverage.status === 'fail') failed.push('coverage');
    if (summary.gates.link.status === 'fail') failed.push('link');
    if (summary.gates.grounding.status === 'fail') failed.push('grounding');
    if (summary.gates.determinism.status === 'fail') failed.push('determinism');
    if (summary.gates.confidence.status === 'fail') failed.push('confidence');
    if (summary.gates.monorepo.status === 'fail') failed.push('monorepo');
