...
/**
 * Lexicon structure: canonical verb → array of synonyms
 */
...
export class LexiconLoader {
  ...
  load(path: string): void {
    try {
      ...
      throw new Error(`Failed to load lexicon from ${path}: ${(error as Error).message}`);
    }
  }
...
let defaultLoader: LexiconLoader | null = null;
 ...
 */
function getDefaultLoader(): LexiconLoader {
  if (!defaultLoader) {
