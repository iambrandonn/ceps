...
function finalizeEntry(pending: PendingEntry | null, result: AnswerParseResult): void {
  if (!pending) return;
 ...
 */
export function parseAnswers(markdown: string, _options: AnswerParseOptions = {}): AnswerParseResult {
  const lines = markdown.split(/\r?\n/);
  const result: AnswerParseResult = { entries: [], errors: [], warnings: [] };
  let current: PendingEntry | null = null;

  const pushError = (line: number, message: string, raw: string) => {
 ...
 */
export function parseAnswersFromFile(filePath: string, options: AnswerParseOptions = {}): AnswerParseResult {
...
export function ingestAnswers(
  kb: KnowledgeBase,
  parseResult: AnswerParseResult,
  options: AnswerIngestionOptions = {}
): AnswerIngestionReport {
  const maxAnswerLength = options.maxAnswerLength ?? 2000;
  const now = options.now ?? (() => new Date().toISOString());

  const report: AnswerIngestionReport = {
    validAnswers: [],
    invalidEntries: [],
    unknownQids: [],
    warnings: [],
  ...
  const unknownQids = new Set<string>();
  ...
  for (const entry of parseResult.entries) {
    ...
    try {
      ...
      const message = error instanceof KBError ? error.message : (error as Error).message;
