...
import { computeImpactReport, type ImpactScopeOptions } from '../impact-scope.js';

function insertEntity(kb: KnowledgeBase, id: string, filePath: string, overrides: Partial<Parameters<typeof createEntity>[0]> = {}) {
  kb.insertEntity(
    createEntity({
      id,
      kind: 'function',
...
function insertOpenQuestion(kb: KnowledgeBase, qid: string, entityId: string): void {
...
describe('computeImpactReport', () => {
  ...
  it('honors hop caps and records excluded nodes', () => {
    ...
    const options: ImpactScopeOptions = { maxHops: 1 };
  ...
  it('emits high-usage warning when approaching node cap without excluding nodes', () => {
    ...
    expect(report.diagnostics.warnings).toContain(
      'Impact traversal visited 4/4 nodes (100% of maxNodes). Consider increasing --finalize-max-nodes or using --finalize-scope full.'
    );
  });
  ...
  it('matches the tiny-react baseline impact report fixture', () => {
    const fixtureRoot = path.resolve(
      path.join('tests', 'fixtures', 'phase5', 'baseline', 'tiny-react')
    );
    const qids = JSON.parse(fs.readFileSync(path.join(fixtureRoot, 'qids.json'), 'utf8'));
    const expected = JSON.parse(
      fs.readFileSync(path.join(fixtureRoot, 'impact.report.json'), 'utf8')
    );
    ...
    const report = computeImpactReport(
      kb,
      qids.map((item: { qid: string }) => item.qid)
    );
