...
import type { ReanalysisResult } from '../reanalysis.js';
...
describe('patchSpecificationFiles', () => {
  it('updates impacted specs, removes QIDs, and inserts finalization summaries', () => {
    ...
    const expectedSpec = fs.readFileSync(
      path.join(FIXTURE_ROOT, 'expected/root.spec.md'),
      'utf8'
    );
    const expectedDirSpec = fs.readFileSync(
      path.join(FIXTURE_ROOT, 'expected/src/spec.md'),
      'utf8'
    );
    const expectedSummary = JSON.parse(
      fs.readFileSync(path.join(FIXTURE_ROOT, 'finalization.summary.json'), 'utf8')
    );
  ...
  it('reports failures when anchors are missing and preserves unresolved QIDs', () => {
    ...
    expect(fs.readFileSync(path.join(workdir, 'spec.md'), 'utf8')).toEqual(
      fs.readFileSync(path.join(FIXTURE_ROOT, 'spec.md'), 'utf8')
    );
  });
  ...
  it('updates multiple entities within the same spec file without index drift', () => {
    ...
    const buttonBaselineChunk: BehaviorChunk = {
      id: 'chunk-raZEDxU4v6',
    ...
    const reanalysis: ReanalysisResult = {
      updatedChunks,
      failedEntities: [],
      warnings: [],
      metrics: {
    ...
    expect(new Set(report.patchedFiles[0].sectionsUpdated.map((section) => section.entityId))).toEqual(
      new Set(['raZEDxU4v6', 'A9m2IW5nts', 'QuVnACVfXW'])
    );
  ...
  it('is idempotent when rerun in deterministic mode with existing summaries', () => {
    ...
    const expectedRoot = ensureTrailingNewline(
      fs.readFileSync(path.join(FIXTURE_ROOT, 'expected/root.spec.md'), 'utf8')
    );
    const expectedDir = ensureTrailingNewline(
      fs.readFileSync(path.join(FIXTURE_ROOT, 'expected/src/spec.md'), 'utf8')
    );
  ...
  it('reports spec-missing failures while still resolving available QIDs', () => {
    ...
    const reanalysis: ReanalysisResult = {
      updatedChunks: new Map<string, BehaviorChunk>([
        [
          'QuVnACVfXW',
          {
            id: 'chunk-QuVnACVfXW',
...
function ingestFixtureAnswers(kb: KnowledgeBase, workdir: string): void {
...
function loadImpactReport(): ImpactReport {
  return JSON.parse(fs.readFileSync(path.join(FIXTURE_ROOT, 'impact.report.json'), 'utf8'));
}

function loadReanalysisResult(): ReanalysisResult {
...
function normalizeSpecContent(value: string): string {
