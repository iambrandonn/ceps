...
import type { Validator, GroundingDiagnostic } from '../../validation/types.js';
...
describe('reanalyzeEntities', () => {
  ...
  function seedEntity({
    entityId = 'entity-1',
    answer,
    chunkText = 'Original placeholder',
    confidence = 'Low'
  }: {
    entityId?: string;
    answer?: string;
    chunkText?: string;
    confidence?: BehaviorChunk['confidence'];
  } = {}) {
  ...
  it('replaces chunk text with provided answer and marks High confidence', async () => {
    ...
    expect(result.updatedChunks.get('entity-1')?.textDraft).toBe(
      'Resolved behaviour details\n- Accepts props\n- Returns markup'
    );
